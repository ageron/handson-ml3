...
models["rf"] = make_pipeline(
    preprocessing, RandomForestRegressor(random_state=42, n_jobs=-1)
)
models["gbr"] = make_pipeline(preprocessing, GradientBoostingRegressor(random_state=42))
if XGBOOST_AVAILABLE:
    models["xgb"] = make_pipeline(
        preprocessing, xgb.XGBRegressor(random_state=42, n_jobs=-1)
    )
...
for name, model in models.items():
    scores = -cross_val_score(
        model, X_train, y_train, scoring="neg_root_mean_squared_error", cv=5, n_jobs=-1
    )
...
for key, search in searches.items():
    ...
    scores = -cross_val_score(
        best, X_train, y_train, scoring="neg_root_mean_squared_error", cv=5, n_jobs=-1
    )
...
final_estimators = [
    (name, est.named_steps[list(est.named_steps.keys())[-1]])
...
stack = StackingRegressor(
    estimators=[
        ...
        if name in ["rf", "gbr"] and name in best_estimators
    ],
...
boot_res = stats.bootstrap(
    [squared_errors], rmse_from_sq, confidence_level=0.95, random_state=42
)
